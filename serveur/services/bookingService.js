...
export default function bookingServiceFactory({bookingRepository, userRepository}) {
    return Object.freeze({
        addBooking, listBookings, listTeacherAvailability, addUserBooking
    });
    ...
    async function addBooking({id, ...changes} = {}) {
        ...
        if (existing.booking.length >= 1) return {message: "You have already an open booking !"};
