...
export default function userControllerFactory() {
    return Object.freeze({
        registerUser, logInUser, patchUser, getAllUsers, getUsername,
    ...
    async function getUsername(httpRequest) {

        const {_id: id} = httpRequest.user;

        try {
            const user = await userService.getUserByUsername({pseudo: httpRequest.params.username, id});
            if (!user) {
