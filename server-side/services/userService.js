...
export default function userServiceFactory({userRepository, tokenRepository}) {
    return Object.freeze({
        ...
        getUser, editScore, resendEmail, verifyUser, getUserByUsername
    });
    ...
    async function listUsers() {
        ...
        return users.filter(x => x.username !== 'admin' && x.username !== 'teacher');
    }
    ...
    async function getUserByUsername({pseudo}) {
        ...
        if (!pseudo) return {message: 'You must supply a username.'};

        return userRepository.findPseudo({pseudo});
